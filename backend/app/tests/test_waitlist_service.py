...
async def test_add_to_waitlist(mock_db, patch_dependencies):
    ...
    with patch("app.services.seat_management_service.SeatManagementService.get_available_seats", return_value=2):
...
async def test_check_in_party(mock_db, patch_dependencies):
    ...
    with patch("app.services.waitlist_service.WaitlistService.simulate_service", new_callable=AsyncMock) as mock_simulate_service:
