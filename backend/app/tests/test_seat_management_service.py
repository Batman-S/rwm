...
async def test_get_available_seats():
    with patch("app.redis_client.get_redis_client", new_callable=AsyncMock) as mock_get_redis_client:
        ...
        available_seats = await SeatManagementService.get_available_seats(mock_redis_client)
...
async def test_decrement_available_seats():
    with patch("app.redis_client.get_redis_client", new_callable=AsyncMock) as mock_get_redis_client:
        ...
        party_size = 4
        await SeatManagementService.decrement_available_seats(mock_redis_client, party_size)
